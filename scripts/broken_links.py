...
# Known WAF / anti-bot signatures, used to identify what's behind 403 responses
# on same-host URLs. Order matters: more specific first.
WAF_SIGNATURES = {
    "Security Pro (PrestaShop)": {
        "headers": [],
        "body": ["security pro", "ps-security", "security-pro"],
    },
    "Cloudflare": {
    ...
    "Sucuri": {
        ...
        "body": ["sucuri website firewall", "this website is using a security service"],
    },
    "Imperva / Incapsula": {
    ...
    "Akamai": {
        "headers": ["akamai-x-cache", "x-akamai"],
...
def detect_waf_signature(headers: dict, body: str) -> str | None:
    """Return the name of the WAF/anti-bot if a signature matches, else None."""
    body_lower = (body or "").lower()
    headers_lower = {(k or "").lower(): (v or "") for k, v in (headers or {}).items()}
    headers_str = " ".join(f"{k}: {v}" for k, v in headers_lower.items()).lower()
    for name, sigs in WAF_SIGNATURES.items():
        for h in sigs["headers"]:
            if h.lower() in headers_lower or h.lower() in headers_str:
                return name
        for b in sigs["body"]:
            if b in body_lower:
                return name
...
def probe_403_body(url: str, timeout: int = 10) -> tuple[dict, str]:
    """Fetch the 403 response with GET to capture headers + body for WAF detection.
    Returns ({headers}, body_snippet)."""
    try:
        resp = safe_get(url, timeout=timeout, headers=HEADERS, allow_redirects=False)
        return dict(resp.headers), (resp.text or "")[:2000]
    except Exception:
        return {}, ""
...
def check_broken_links(url: str, internal_only: bool = False,
    ...
    result = {
        ...
        "timeout": [],
        "security_blocked": [],
        "security_blocked_provider": None,
        "security_blocked_probe": None,
        "healthy": 0,
        "summary": {},
        "issues": [],
        "error": None,
    ...
    if len(pending_403_internal) >= 5:
        ...
        # Probe the first 403 to identify the WAF (single extra HTTP call)
        first_403 = pending_403_internal[0]["url"]
        probe_headers, probe_body = probe_403_body(first_403, timeout=timeout)
        waf_name = detect_waf_signature(probe_headers, probe_body)
        result["security_blocked_provider"] = waf_name or "unknown"
        result["security_blocked_probe"] = {
            "url": first_403,
            "headers_sample": {k: v for k, v in probe_headers.items() if k.lower() in (
                "server", "x-powered-by", "cf-ray", "x-sucuri-id", "x-iinfo",
                "x-cdn", "akamai-x-cache", "x-amzn-waf", "cf-cache-status", "cf-mitigated",
            )},
            "body_excerpt": probe_body[:500],
        }
    else:
    ...
    result["summary"] = {
        ...
        "security_blocked_provider": result.get("security_blocked_provider"),
    }
    ...
    if result["security_blocked"]:
        provider_label = (
            f"WAF identifié : {waf_name}" if waf_name
            else "WAF non identifié (pattern compatible avec Security Pro, Cloudflare bot fight, Wordfence, etc.)"
        )
        result["issues"].append(
            f"ℹ️ {len(result['security_blocked'])} internal link(s) return 403 — {provider_label}. "
            "Ces URLs ne sont pas réellement cassées pour un navigateur humain ; "
            "soit whitelister l'user-agent d'audit, soit ignorer ce signal."
        )
